...
class InsuranceDataset(Dataset):
    def __init__(self,
        ...
        assert path.exists(path.join(root, "metadata.json")), "metadata file not found."
    ...
    @staticmethod
    def incomeEncoder(X):
        log_inc = X.astype(float).apply(lambda x: max(1, x)).apply(math.log)
        return pd.DataFrame(log_inc)
    ...
    def encode_data(self):
        ...
        log.info("metadata is read.")
  
        log.info("nan resolution (Off).")
        ...
        log.info("income log-transform.")
        data['income'] = self.incomeEncoder(data['income'])

        sub_columns = metadata["to_label_encoder"]    
        ...
        sub_columns = metadata["to_minmax_encoder"]
        log.info("min-max-transform.")
        for col_name in tqdm.tqdm(sub_columns):
            col_data = data[col_name]
            col_fit, col_data = self.label_fit_transform(col_data.to_numpy().reshape(-1,1), enc_type="min_max")
            self.encoder_fit[col_name] = col_fit
            data[col_name] = col_data


        log.info("Quant transform")
        sub_columns = metadata["to_quantize"]
        for col_name in tqdm.tqdm(sub_columns):
        ...
        columns_to_select = metadata["columns"]
